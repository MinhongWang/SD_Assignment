...
def playerOneInitial():
    ...
    pO['discard'] = []
    pO['active'] = []
    #player draw the first new hand from their deck
...
def mainDeckCentralLineInitial():
    ...
    central['active'] = []
    #move cards from main deck to central line
    ...
    while count < max:
        ...
        count = count + 1
#Function that get the attack value of all cards player one hold
#Used for card exhuasted game end
def getPlayerHoldStrength(pO):
    if (len(pO['hand']) >0 ):
        for x in range(0, len(pO['hand'])):
            pO['deck'].append(pO['hand'].pop())    
    if (len(pO['active']) >0 ):
        for x in range(0, len(pO['active'])):
            pO['deck'].append(pO['active'].pop())
    if (len(pO['discard']) >0 ):
        for x in range(0, len(pO['discard'])):
            pO['deck'].append(pO['discard'].pop())
    playerHoldStrength=0
    if (len(pO['deck'])>0):
        for card in pO['deck']:
            playerHoldStrength=playerHoldStrength+card.get_attack()
    return playerHoldStrength
#Function that get the attack value of all cards computer hold.
#Used for card exhausted game end
def getComputerHoldStrength(pC):
    if (len(pC['hand']) >0 ):
        for x in range(0, len(pC['hand'])):
            pC['deck'].append(pC['hand'].pop())
    if (len(pC['active']) >0 ):
        for x in range(0, len(pC['active'])):
            pC['deck'].append(pC['active'].pop())
    if (len(pC['discard']) >0 ):
        for x in range(0, len(pC['discard'])):
            pC['deck'].append(pC['discard'].pop())
    computerHoldStrength=0
    if (len(pC['deck'])>0):
        for card in pC['deck']:
            computerHoldStrength=computerHoldStrength+card.get_attack()
    return computerHoldStrength
...
if __name__ == '__main__':
    ...
    playGame = raw_input('Do you want to play a game?(Y=Yes, N=No):').upper()
    ...
    opponentChoice = raw_input("Do you want an aggressive (A) opponent or an acquisative (Q) opponent?:").upper()
    while (opponentChoice!='A' and opponentChoice!='Q'):
        print("\nWrong input, please input again! (A=Aggressive, Q=Acquisative)")
        opponentChoice = raw_input("Do you want an aggressive (A) opponent or an acquisative (Q) opponent?:").upper()
    aggressive = (opponentChoice=='A')
    acquisative = (opponentChoice=='Q')

    print "Game Start".center(70,'-')        
    while continueGame:
        money = 0
        attack = 0
        #Loop until computer end turn or game end
        while True:
            if not continueGame:
                break
            ...
            if act == 'Q':
                print "Your input is: quit the game".center(70,'-')
                print("\nThank you!")
                sys.exit()
            #If player input P to play all cards
            if act == 'P':
                if(len(pO['hand'])>0):
                    print "Your input is: move all cards to active area".center(70,'-')
                    for x in range(0, len(pO['hand'])):
                        card = pO['hand'].pop()
                    ...
                    print "There is not card in hand!".center(70,'-')
            #If player input a number to play only one card
            if act.isdigit():
                if( int(act) < len(pO['hand'])):
                    buyOneCard = "Your input is: move card ["+act+"] to active area"
                    print buyOneCard.center(70,'-')
            ...
            if (act == 'B'):
                print "Your input is: start buying cards".center(70,'-')
                ...
                while money > 0:
                    ...
                    if central['activeSize'] == 0:
                        ...
                        if pO['health'] > pC['health']:
                            ...
                            pHT = getPlayerHoldStrength(pO)
                            pCT = getComputerHoldStrength(pC)
                            if pHT > pCT:
                                ...
                                print "Draw"
                        break                    
                print "End buying cards".center(70,'-')
            #If player input A to attack
            if act == 'A':
                print "Your input is: attack opponent".center(70,'-')
                if attack == 0:
                    print "Sorry! You cannot attack opponent since your attack value is zero.".center(50)
                    print "Attack end".center(70,'-')
                else: 
        ...
        if continueGame:
            print "Computer Turn Start".center(70,'-')
        ...
        if continueGame:
            ...
            for x in range(0, pC['handsize']):
                        ...
                        card = pC['deck'].pop()
                        pC['hand'].append(card)
            print "Computer Turn Ending".center(70,'-')
